...
import { api, apiClient, ApiError, ApiResponse, cacheManagement, RequestConfig } from '../../lib/api/api-client';
 ...
 */
export function useSearch<T = unknown>(query: string, endpoint: string, debounceMs = 300) {
...
export function useUserOperations() {
  ...
  const updateSettings = useMutation(
    (data: Record<string, unknown>) => api.user.updateSettings(data),
    {
      invalidateCache: ['/api/user'],
    }
  );
...
export function useInfiniteApi<T = unknown>(
  ...
  const { loading, error, fetch } = useApi<T[]>(
    getUrl(currentPage),
    {
      ...apiOptions,
      immediate: false,
